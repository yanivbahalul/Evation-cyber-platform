...
import { deltaMs, learningHints, mlHints, severityColor, shortTrace, summarizeMl, trapLabel, type MlSummary } from '@/lib/attackIntel'
...
const MlEventBadge = ({ ml }: { ml: MlEnrichment }) => {
    ...
    <div className="flex flex-col gap-4 h-full min-h-0">
      ...
      {timeline && (timeline.profile || timeline.events.length > 0) && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 min-h-0 flex-1">
          <ProfileCard profile={timeline.profile} fallbackIp={ipInput} hints={hints} ml={ml} />
