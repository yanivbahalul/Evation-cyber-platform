    ...
    return (req, res, next) => {
        ...
        res.on('finish', async () => {
            ...
            if (!req.isLogFlooding) {
                // 1. Sub-second WebSocket alert to Yaniv's Blue Team dashboard.
                // Keep this non-blocking so DB issues don't delay live alerts.
                SocketService.emitLiveAlert(attackData);

                // 2. Persist + console log via LoggerService (Mongoose write to attack_events).
                LoggerService.logAttack(attackData).catch((err) => {
                    console.error('❌ [Telemetry] LoggerService.logAttack failed:', err?.message || err);
                });
            }
